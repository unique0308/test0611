# Claude Code 工作约定(工程内副本)

> ⚠️ **完整版在 `../MVP跟踪文档/CLAUDE.md`**(v1.3,Day 7 末)。每次新 session 第一件事读完整版,这里只是工程内的快速指针。
...
1. `../MVP跟踪文档/CLAUDE.md` § 0 项目位置 + 快速命令 / § 1 阅读顺序 / § 2 工作纪律
...
⚠️ **项目尚未 git init**。CLAUDE.md 第 10.2 节对账步骤里"看 git log"暂时跳过,只看文档变更日志。
后续 `git init` 时:`.gitignore` 已写好,`.env.local` / `uploads/` / `.next/` / `.npm-cache/` 都已忽略。

## 收工纪律(必做)

每次 session 收工执行 `../MVP跟踪文档/CLAUDE.md` 第 7.3 节 5 步流程。
